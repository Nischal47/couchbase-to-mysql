...
public class CouchbaseToMysqlImplementation implements CouchbaseToMysql {
    ...
    List<String> couchbaseDataBill = null;
...
//            couchbaseDataBill = couchbaseDatabase.query("WHERE type = \"bill\"", JsonObject.create());
...
//            couchbaseDataTableCategory = couchbaseDatabase.query("WHERE type = \"table_category\"", JsonObject.create());
        ...
        if(!CollectionUtils.isEmpty(couchbaseDataBill)){
            upsertBillToMysql(couchbaseDataBill);
        }
    }
    ...
    @Override
    public void upsertBillToMysql(List<String> data) {
        CouchbaseDatabaseImplementation couchbaseDatabase = null;
        MysqlDatabaseImplementation mysqlDatabase = null;
        ...
        try{
            ...
            for (String record : data){
                JSONObject jRecord = new JSONObject(record);
                mysqlDatabase.executeUpdate("INSERT INTO `bill` " + Util.buildInsertBill(jRecord)
                        + " ON DUPLICATE KEY UPDATE " + Util.buildUpdateBill(jRecord, jRecord.getString("id")));
            }
            List<String> mysqlData = mysqlDatabase.executeQuery("select * from `bill`");
            int i = 1;
            for (String mysql:mysqlData){
                JSONObject jRecord = new JSONObject(mysql);
                couchbaseDatabase.setBucket("getting-started-bucket");
                List<String> cb = couchbaseDatabase.query(" WHERE id =\""+jRecord.getString("id")+"\"");
                if(cb == null || cb.isEmpty()) {
                    mysqlDatabase.executeUpdate("DELETE FROM `bill` WHERE id = \"" + jRecord.getString("id") + "\"");
                    System.out.println("Deleted");
                }
            }
        } finally {
